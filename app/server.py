...
createdcall = retell.call.create(
     from_number="+17869980713", # replace with the number you purchased
    to_number="+15612695870",  # replace with the number you want to call
)


print("hello", createdcall)
...
async def websocket_handler(websocket: WebSocket, call_id: str):
    try:
        ...
        async def handle_message(request_json):
            try:
                tmp = request_json['transcript'][-1]['content']
                request_json['transcript'][-1]['content'] = tmp.replace("*", "")
            except Exception as err:
                print(f"Error in register call: {err}")
